    ...
    if request.user.is_authenticated:
        if user == request.user.username:
            if request.method == 'POST':
                ...
                Contact.objects.filter(user=request.user, pk__in=yes_list_id).update(is_public='بله')
                Contact.objects.filter(user=request.user, pk__in=no_list_id).update(is_public='خیر')
            return redirect(request.META.get("HTTP_REFERER"))
        else:
            return HttpResponse('شما مجاز به این عمل نیستیند')
    return redirect(reverse('phone_book_accounts:login'))
        ...
        if user == request.user.username:
            ...
            if add_file_form.is_valid():
                try:
                    ...
                    if len(cells) != 0:
                        error_text += ' این خانه ها ' + f'{cells}' + ' از ستون ' + f'{column}' + ' باید کوچکتر از ' + f'{con}' + ' باشد' + '\n'
                    column = 'phone'
                    con = 100
                    s = df[column].str.len().between(0, con)
                    cells = list(s[s == False].index)
